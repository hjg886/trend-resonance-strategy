...
# v5.0 #167：动态扩展 UNIVERSE 至全部可用个股数据（突破硬编码8只瓶颈；财报缺失经 layer1_stock 中性通过代理）
#   v5.0 固化基线：默认开启 BT_UNIV_V5=1（#167个股通道打通是v5.0核心特性；关闭则退化为硬编码8只池=六年0笔根因）
if os.environ.get("BT_UNIV_V5", "1") == "1":
    _dd = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data")
    for _fn in os.listdir(_dd):
        if _fn.startswith("stk_") and _fn.endswith(".csv"):
            _code = _fn[4:-4]
            if _code not in UNIVERSE:
                UNIVERSE[_code] = ("stk_" + _code, _code, "stock", None)
...
# 得分门槛（参数集中管理·全部由 env 读取，消除"静默失效"——P2-03修复）
# 非R10模式(legacy)使用以下默认值；R10模式(默认开)使用下方 R10_GATE_* 默认值
SCORE_GATE = float(os.environ.get("BT_SCORE_GATE", "70.0"))       # 常规(S1) 默认70
S2_GATE = float(os.environ.get("BT_S2_GATE", "75.0"))             # S2减半更严 默认75
OBS_GATE = float(os.environ.get("BT_OBS_GATE", "72.0"))           # 观察通道/S3 默认72
# R10 重构分轨门槛（v4.7-R10, 默认开; 原硬编码75/73/78→改为env可读, 优先级高于legacy——P2-03）
R10_GATE_S1 = float(os.environ.get("BT_R10_GATE_S1", "75.0"))
R10_GATE_S2 = float(os.environ.get("BT_R10_GATE_S2", "75.0"))
R10_GATE_OBS = float(os.environ.get("BT_R10_GATE_OBS", "78.0"))
R10_GATE_IND_S1 = float(os.environ.get("BT_R10_GATE_IND_S1", "73.0"))  # 行业ETF得分天然偏低-2分
R10_GATE_IND_S2 = float(os.environ.get("BT_R10_GATE_IND_S2", "73.0"))
# v5.0 #167 个股通道独立门槛（默认70; 个股24因子系统性低于ETF约5-10分，统一75结构性饿死个股通道）
STOCK_GATE = float(os.environ.get("BT_STOCK_GATE", "70.0"))
# 进攻线专用: 统一得分门槛（>0时覆盖所有状态门槛为该值, 用于候选B严格筛选）
ATTACK_SCORE = float(os.environ.get("BT_ATTACK_SCORE", "0.0"))
# v5.0 #167 个股通道打通变体：R值去硬否决(>0.35才否决,0.25-0.35交MIN③降权) + 个股观察/S3得分门槛80→75
STOCKCHANNEL_V5 = os.environ.get("BT_STOCKCHANNEL_V5", "1") == "1"  # v5.0 固化基线：默认开启（#167个股通道打通：UNIVERSE扩至83只+R值去硬否决）
# v5.0 #167 个股专属配额：反对过度保守=变相不参与——确保个股通道不被高分ETF结构性饿死
#   开放式漏斗哲学落地：入口放宽筛选后，出口按"个股≥STOCK_QUOTA只"保障收益来源(归因⑥ETF亏/个股盈)不被挤占
#   STOCK_QUOTA=0 关闭(纯得分排序, 旧行为); ≥1 时个股优先占用前STOCK_QUOTA个建仓名额, ETF填充剩余至5只
...
VOLGATE_ON = os.environ.get("BT_VOLGATE", "1") == "1"  # v5.0 固化基线：默认开启（R-04波动率环境双闸门，建仓第一前置条件）
...
S1CONFIRM_ON = os.environ.get("BT_S1CONFIRM", "1") == "1"  # v5.0 固化基线：默认开启（R-05 S1突破确认制）
# R-06 止损带宽时间分档: 持有≤11日5% / >11日3%（engine.py tenor_on）
EP1TENOR_ON = os.environ.get("BT_EP1TENOR", "1") == "1"  # v5.0 固化基线：默认开启（R-06止损带宽时间分档≤11日7%/>11日4%）
...
EP1_BAND_SHORT = float(os.environ.get("BT_EP1_BAND_SHORT", "7.0"))  # v5.0 固化基线：≤11日带宽7%（P0d/JOINT回测验证最优，对齐主文档R-06）
EP1_BAND_LONG = float(os.environ.get("BT_EP1_BAND_LONG", "4.0"))  # v5.0 固化基线：>11日带宽4%（P0d/JOINT回测验证最优，对齐主文档R-06）
...
# ============ 去冗余·冗余2：趋势位置合并变体（2026-08-24 提前回测, 默认0=基线原逻辑） ============
# 范围: 个股+宽基ETF（行业ETF保留R3 z-score, 不合并）
# "0"=原逻辑(等价V0) / "1"=温和(②④⑥ C→25) / "2"=标准(②⑥C→25, ④C→0) /
# "3"=严格(②④⑥ C→0) / "4"=严格+第2灯(第4灯)仅A档
TP_VARIANT = os.environ.get("BT_TP_VARIANT", "0")


# ============ P1-03 滑点冲击测试参数 ============
CAPITAL = float(os.environ.get("BT_CAPITAL", "1000000.0"))    # 初始资金(默认100万)
SLIP_IMPACT = float(os.environ.get("BT_SLIP_IMPACT", "0.0"))   # 规模冲击成本系数(√(AUM/100万)缩放; 0=无冲击)
OUTFIX = os.environ.get("BT_OUTFIX", "")                       # 输出文件后缀(非空→results_{in,oos}_{fix}.json, 避免覆盖基准)

# ============ v5.0 固化基线 run-manifest（可复现性·对应复核 P0-03/P1-02） ============
# 每次运行将实际生效的全部 BT_* 环境变量写入 OUT/run_manifest.json，
# 杜绝"默认关闭静默生效"导致的不可复现与绕过；缺失关键开关时显式告警。
def dump_run_manifest():
    try:
        manifest = {k: v for k, v in os.environ.items() if k.startswith("BT_")}
        manifest["_v5_hardened_defaults"] = {
            "BT_STOCKCHANNEL_V5": "1 (#167 个股通道打通)",
            "BT_VOLGATE": "1 (R-04 波动率双闸门)",
            "BT_S1CONFIRM": "1 (R-05 S1确认制)",
            "BT_EP1TENOR": "1 (R-06 止损带宽时间分档)",
            "BT_EP1_BAND_SHORT": "7.0 (≤11日, P0d固化)",
            "BT_EP1_BAND_LONG": "4.0 (>11日, P0d固化)",
            "BT_R10GATES": "1 (R10分轨门槛, 由BT_R10_GATE_*读取)",
            "BT_STOCK_GATE": "70.0 (#167 个股独立门槛)",
        }
        # P2-03 透明化：记录当前生效的得分门槛(优先级 R10模式→BT_R10_GATE_*/#167；legacy→BT_SCORE_GATE等)
        if R10_GATES_ON:
            manifest["_effective_gates"] = {
                "mode": "R10(默认)", "S1": R10_GATE_S1, "S2": R10_GATE_S2, "OBS": R10_GATE_OBS,
                "ind_etf_S1": R10_GATE_IND_S1, "ind_etf_S2": R10_GATE_IND_S2, "stock_#167": STOCK_GATE,
            }
        else:
            manifest["_effective_gates"] = {
                "mode": "legacy", "S1": SCORE_GATE, "S2": S2_GATE, "OBS": OBS_GATE,
                "ind_etf_S1": ETFI_GATE_S1, "ind_etf_S2": ETFI_GATE_S2, "ind_etf_OBS": ETFI_GATE_OBS,
            }
        manifest["_slippage"] = {"CAPITAL": CAPITAL, "SLIP_IMPACT": SLIP_IMPACT, "OUTFIX": OUTFIX}
        with open(os.path.join(OUT, "run_manifest.json"), "w", encoding="utf-8") as _fh:
            json.dump(manifest, _fh, ensure_ascii=False, indent=2)
        print("[run_manifest] 已写入 OUT/run_manifest.json")
    except Exception as _e:
        print(f"[WARN] run_manifest 写入失败: {_e}")
dump_run_manifest()
...
def run_backtest(panels: dict, fins: dict, days: list, period: tuple, label: str) -> dict:
    tre = TreState()
    pf = Portfolio(init_cash=CAPITAL)
    pf.slip_impact = SLIP_IMPACT
    ...
    for i, date in enumerate(days):
        ...
        if not melt_block and not comp_p9 and not vol_block:
            ...
            # v5.0 #167 个股通道打通 + A股为主ETF为辐：个股整体优先于ETF处理,
            #   STOCK_QUOTA=个股最大持仓只数(防集中度), ETF补位至总5只
            #   —— 落地"反对无意义高门槛卡死(过度保守=变相不参与), 但提收益基础上科学控风险"
            #   个股仍须通过全部前置过滤(层级1/RPS/R值/TRE/六灯/MIN/F04/出口风控), 仅保障不被高分ETF结构性挤占
            if STOCKCHANNEL_V5 and STOCK_QUOTA > 0:
                _stk = [c for c in cands if UNIVERSE[c][2] == "stock"]
                _etf = [c for c in cands if UNIVERSE[c][2] != "stock"]
                cands = _stk + _etf   # A股为主: 个股先于ETF建仓
            for code in cands:
                ...
                # v5.0 #167：个股通道 RPS 硬门槛由≥30放宽至≥20（仅排除最弱后20%动量；与评分内RPS因子去重，消除过滤过载）
                rps_floor = 20 if (STOCKCHANNEL_V5 and ptype == "stock") else 30
                rps_ok = row.get("RPS60", 50) >= rps_floor
                if not rps_ok:
                    # v5.0 #167: 个股通道 RPS 降权而非否决(核心哲学: 禁止硬否决, 交24因子评分排序自然降权)
                    if not (STOCKCHANNEL_V5 and ptype == "stock"):
                ...
                if ptype != "stock":
                    ...
                    _r = row.get("R", 99)
                    if STOCKCHANNEL_V5:
                        # v5.0 #167：R值去硬否决（对齐L60/L77“性价比类指标应降权而非否决”哲学）；
                        #   仅极端劣化 R>0.35 否决；0.25<R≤0.35 不否决、交MIN③降权处理（仍按B级参与灯位）
                ...
                # v5.0 #167: 个股通道 R值C级不再硬否决(经MIN③降权, 见scoring.min_nine③); 仅旧逻辑/ETF保持
                if r_level == "C" and not (STOCKCHANNEL_V5 and ptype == "stock"):
                ...
                if R10_GATES_ON:
                    ...
                    #   回测证据：降门槛期望更高(+0.77%)；行业ETF得分天然偏低保持-2分(73 vs 75)
                    #   P2-03修复：门槛全部由 env(BT_R10_GATE_*) 读取，移除硬编码，消除"env静默失效"
                    g_s1 = R10_GATE_IND_S1 if ind_etf_flag else R10_GATE_S1
                    g_s2 = R10_GATE_IND_S2 if ind_etf_flag else R10_GATE_S2
                    g_obs = R10_GATE_OBS
                    if STOCKCHANNEL_V5 and ptype == "stock":
                        # v5.0 #167：个股通道独立门槛=70（默认BT_STOCK_GATE, env可读）
                        g_s1 = g_s2 = g_obs = STOCK_GATE
                else:
                    # legacy模式: 使用 BT_SCORE_GATE/S2_GATE/OBS_GATE + 行业ETF分轨(BT_ETFI_GATE_*)
                ...
                # v5.0 A股为主: 个股持仓达 STOCK_QUOTA 上限后, 该股不再建仓(让ETF补位至5只)
                if STOCKCHANNEL_V5 and ptype == "stock" and STOCK_QUOTA > 0:
                    n_stk = sum(1 for c2 in pf.holdings if UNIVERSE[c2][2] == "stock")
                    if n_stk >= STOCK_QUOTA:
                        stats["intercept"]["个股配额满"] += 1; continue
                ...
                # 【去冗余·冗余2】SSOT趋势位置档位：六灯第2灯/七灯第4灯 + MIN②④⑥ 引用同一结果
                tp_grade = trend_position(row, tre.state) if TP_VARIANT != "0" else None
                if ptype == "stock":
                    ok_l, lights = six_lights(row, tre.state, tre.obs_active, tre.state == "S3",
                                              n_hold, n_same, beta, cap, open_pct, r_level, tp_grade)
                else:
...
def main():
    ...
    suffix = f"_{OUTFIX}" if OUTFIX else ""
    with open(os.path.join(OUT, f"results_in{suffix}.json"), "w", encoding="utf-8") as fh:
        json.dump(res_in, fh, ensure_ascii=False, indent=1, default=str)
    with open(os.path.join(OUT, f"results_oos{suffix}.json"), "w", encoding="utf-8") as fh:
        json.dump(res_oos, fh, ensure_ascii=False, indent=1, default=str)
    if OUTFIX:
        print(f"[OUTFIX] 结果已写入 results_in{suffix}.json / results_oos{suffix}.json")
