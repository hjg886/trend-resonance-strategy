# -*- coding: utf-8 -*-
"""
sweep_b2.py —— Track B2 ETF 评分优化扫描（2026-08-26）
在 B1 主引擎(TRE状态收紧, OOS基线≈3.86%)基础上，仅改 ETF 选股评分 etf_score:
  新增估值分位(NAV_PCTL_120) + 低波(VOL20A) 维度，降动量(RPS20/ret20)暴露
扫描: EST_W∈{20,25,30} × VOL_W∈{15,20,25} × MOM_SCALE∈{0.4,0.5,0.6} = 27 组
    + b2_v1(BT_B2_ON=0, 回退V1 etf_score, A/B对照, 应≈B1 3.86%)
验收(主引擎整体): OOS≥3.5% 且 OOS>IS 且 mdd≤12% 且 freq≥6
硬约束: 主引擎整体 OOS 不得较 B1 基线 3.86% 退化 >0.3% (ETF为防守辐, 占比有限, 不应拖累整体)
"""
...
# B1 基线 OOS（已验证 rc2）：主引擎 OOS 3.86%
B1_OOS_BASE = 3.86
GRID = []
for est in (20, 25, 30):
    for vol in (15, 20, 25):
        for mom in (0.4, 0.5, 0.6):
            fix = f"b2_e{est}_v{vol}_m{mom}"
            env = {"BT_B2_ON": "1", "BT_B2_EST_W": str(est),
                   "BT_B2_VOL_W": str(vol), "BT_B2_MOM_SCALE": str(mom)}
            GRID.append((fix, env))
CONFIGS = [("b2_v1", {"BT_B2_ON": "0"})] + GRID
...
def main():
    ...
    for fix, env in CONFIGS:
        try:
            ...
            traceback.print_exc()
    with open(os.path.join(M.OUT, "sweep_b2.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=1)
    print("\n===== Track B2 汇总（验收: OOS_ann≥3.5 且 OOS>IS 且 mdd≤12 且 freq≥6；"
          f"硬约束: OOS 不较 B1基线{B1_OOS_BASE}% 退化>0.3%）=====", flush=True)
    print(f"{'fix':<16}{'OOS_ann':>9}{'OOS_etf':>9}{'IS_ann':>8}{'OOS_mdd':>8}{'trd':>5}"
    ...
    for r in results:
        ...
        print(f"{r['fix']:<16}{r['OOS_ann']:>9}{r['OOS_etf_ann']:>9}{r['IS_ann']:>8}"
        ...
        if ok and hard:
            if best is None or r["OOS_ann"] > best["OOS_ann"]:
                best = r
    print(f"\nDONE {len(results)}/28 配置；最佳 PASS 配置: "
