#!/usr/bin/env python3
"""2026-08-24 收盘总结计算脚本（数据源：收盘快照 kline_close_0824.txt，parts[4]=收盘价）"""
import json, math
from datetime import date

SRC = r'E:\fnOS\文档\证券\中线趋势共振策略\kline_close_0824.txt'
...
for sym,name in idx_spec:
    ...
    print(f"{name}: 收盘{last['close']} 今开{last['open']} 昨收{prev['close']} 涨跌{chg:+.2f}% "
...
print("\n=== 持仓监控 ===")
# 510300：09:15竞价限价≤4.65清仓指令已挂；今日最低4.593/开盘4.678，竞价区间理应成交；实际成交价/量待用户回报
...
print(f"510300: 收盘{l510['close']} 昨收{p510['close']} 涨跌{(l510['close']-p510['close'])/p510['close']*100:+.2f}% "
...
print(f"159650: 收盘{l159['close']} 昨收{p159['close']} 涨跌{(l159['close']-p159['close'])/p159['close']*100:+.2f}% "
...
print(f"药明康德: 收盘{lym['close']} 昨收{pym['close']} 涨跌{(lym['close']-pym['close'])/pym['close']*100:+.2f}% "
      f"MA60={m60ym:.2f}(偏离{devym:+.2f}%) ADX={advym:.1f} vol20={v20ym:.2f}% 上方={abv}")

# 账户估算（假设510300竞价清仓已成交@~4.65保守价；实际待回报）
cash_base=366896.62+44534+95004
bid_assume=4.65
proceeds_remain=h510['shares']*bid_assume
cash_total=cash_base+proceeds_remain
total=cash_total+mkt159
print(f"\n=== 账户估算（清仓后，竞价@假设{bid_assume}）===")
...
print(f"若竞价@4.65: 剩余回收¥{proceeds_remain:,.0f}, 剩余部分亏损¥{(bid_assume-h510['cost'])*h510['shares']:,.0f}")
...
out={'indices':results,'gate':gate,'global_tre':gTRE,'s4_count':s4,
     'hs300_vol20':hs,'m510':{'last':l510['close'],'prev':p510['close'],'e_p1':e_p1,'peak':peak,'dd':dd,
                              'mkt':mkt,'pnl':pnl,'chg':(l510['close']-p510['close'])/p510['close']*100,'hd':hd},
     ...
     'cash_total':cash_total,'total':total,'m159_mkt':mkt159,'bid_assume':bid_assume}
with open(r'E:\fnOS\文档\证券\中线趋势共振策略\close_0824_result.json','w',encoding='utf-8') as f:
    json.dump(out,f,ensure_ascii=False,indent=2,default=str)
print("\n结果保存: close_0824_result.json")
