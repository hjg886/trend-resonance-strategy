...
入场门槛（须同时满足，对齐 run_attack_b.py B3 变体，阶段1锁定 2026-08-18）：
  ...
  ② 简化技术评分 ≥ 70（趋势50+动量30+流动性20）
  ③ 个股三条件：成交额≥5000万 + ATR20P≤10 + 评分≥70（台账记录时应用）
说明：
  ...
  - 扫描池 = cand_meta.json 72 只（补数据后 72/72）；回测基准池 = OBS_POOL 24 只子集，
    对照回测时以 24 只交集为准（见 scan_b3_daily.py）。
"""
...
# ---------------- 市场状态（回测口径） ----------------
import numpy as np
from run_attack_b import load_all, build_panels, compute_tre_states, OBS_POOL
...
def main():
    ...
    for code in codes:
        ...
        with io.open(path, "r", encoding="utf-8-sig") as f:
            data = [{"date": r["date"], "close": float(r["close"]), "volume": float(r["volume"]),
        ...
        if sc is None:
            ...
            continue
        tr, mo, li, total = sc
        rows_out.append((c6, names.get(code, ""), total, tr, mo, li))
    rows_out.sort(key=lambda x: -x[2])
    print(f"{'代码':<7}{'名称':<9}{'总分':>7}  {'信号'}")
    for c6, nm, total, tr, mo, li in rows_out:
        if total >= 70:
            mark = "★评分达标" if ms["open"] else "≥70(环境暂阻)"
        else:
            mark = ""
        print(f"{c6:<7}{nm:<9}{total:>7.1f}  {mark}")
    print("-" * 74)
    sigs = [r for r in rows_out if r[2] >= 70]
    print(f"\n【评分≥70】{len(sigs)} 只 / 全池 {len(rows_out)} 只有效数据 / 缺数据 {len(missing)} 只 / 失效(退市/停牌) {len(stale)} 只")
    if stale:
        for s in stale:
            print(f"  ⚠️ {s}")
    if sigs:
        for c6, nm, total, tr, mo, li in sigs:
            print(f"  {c6} {nm} 总分{total:.1f}（趋势{tr:.1f}/动量{mo:.1f}/流动{li:.1f}）")
    print(f"\n【结论】{'建仓权限开放: 评分≥70 即触发信号' if ms['open'] else '当日禁建仓: ' + ms['permission']}")
    if ms["open"] and sigs:
        print(f"  → 可执行信号 {len(sigs)} 只（按台账流程：次日开盘模拟成交）")
